...
class TestTraining(unittest.TestCase):
    ...
    def test_one_layer_train(self):
        ...
        batched_train_data, batched_train_label = generate_batched_data(
            train_data, train_label, batch_size=128, shuffle=True, seed=1024
        )
        _, train_acc = train(
            1, batched_train_data, batched_train_label, model, optimizer, debug=False
        )
        ...
        _, test_acc = evaluate(
            batched_test_data, batched_test_label, model, debug=False
        )
    ...
    def test_two_layer_train(self):
        ...
        batched_train_data, batched_train_label = generate_batched_data(
            train_data, train_label, batch_size=32, shuffle=True, seed=1024
        )
        _, train_acc = train(
            1, batched_train_data, batched_train_label, model, optimizer, debug=False
        )
